...
public class Storage extends HashMap<String, Object> {
	...
	private static final char CHAR_BACKSLASH = '\\', CHAR_DIVIDER = '|', CHAR_BRACKET_OPEN = '[',
			CHAR_BRACKET_CLOSE = ']', CHAR_NULL = '�';
	private static final char[] escapedChars = { CHAR_BACKSLASH, CHAR_DIVIDER, CHAR_BRACKET_OPEN, CHAR_BRACKET_CLOSE,
			CHAR_NULL };
	private String name;
	...
	public void saveToYAML(File directory) throws IOException {
		File file = new File(directory, this.name + ".yml");
	...
	public void save(StorageType type, File directory, FruitSQL mySQL, String table) throws IOException, SQLException {
		switch (type) {
		case YAML:
			saveToYAML(directory);
			return;
		case MYSQL:
			saveToMySQL(mySQL, table);
			return;
		default:
			throw new NotImplementedException("StorageType '" + type + "' is not yet implemented.");
		}
	}

	public static Storage load(StorageType type, String name, File directory, FruitSQL mySQL, String table)
			throws IOException, SQLException {
		switch (type) {
		case YAML:
			return loadFromYAML(directory, name);
		case MYSQL:
			return loadFromMySQL(mySQL, name, table);
		default:
			throw new NotImplementedException("StorageType '" + type + "' is not yet implemented.");
		}
	}
	...
	public static boolean isSerializedMySQLList(String string) {
		return string.charAt(0) == CHAR_BRACKET_OPEN && string.charAt(string.length() - 1) == CHAR_BRACKET_CLOSE
				&& !isMySQLEscaped(string, string.length() - 1);
	}
