		...
		try {
			...
			if (im instanceof LeatherArmorMeta) {
				...
				if (lore == null || lore.size() < 1) {
					Breakpoint.warn("Error when parsing LeatherArmor: "
							+ (lam.hasDisplayName() ? lam.getDisplayName() : Material.getMaterial(type)));
					return null;
				}

				int minutesLeft = Integer.parseInt(lore.get(0)
						.substring(MessageType.EQUIPMENT_MINUTESLEFT.getTranslation().getValue().length() - 3));
				...
				int minutesLeft = Integer.parseInt(lore.get(1)
						.substring(MessageType.EQUIPMENT_MINUTESLEFT.getTranslation().getValue().length() - 3));
				...
				try {
					...
					if (lore == null || lore.size() < 1) {
						Breakpoint.warn(
								"Error when parsing Equipment: " + (im.hasDisplayName() ? im.getDisplayName() : ""));
						return null;
					}
					...
					Breakpoint
							.warn("Error when parsing equipment: " + (im.hasDisplayName() ? im.getDisplayName() : ""));
					return null;
				}
			} else {
				return null;
