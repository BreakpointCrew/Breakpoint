...
public class Achievement {
	...
	public static ItemStack getButton(int i) {
		...
		im.setDisplayName(
				ChatColor.DARK_AQUA + (i == 0 ? MessageType.MENU_ACHIEVEMENTS_PREVIOUSPAGE.getTranslation().getValue()
						: MessageType.MENU_ACHIEVEMENTS_NEXTPAGE.getTranslation().getValue()));
		is.setItemMeta(im);
		return is;
	}
	...
	public static Object[] getAchievementsInOrder(BPPlayer bpPlayer) {
		...
		List<Object[]> achieved = new ArrayList<>();
		List<Object[]> notAchieved = new ArrayList<>();
		for (AchievementType at : order) {
	...
	public static List<String> breakUpText(String s) {
		...
		for (String element : ss) {
			result.set(j, result.get(j) + " " + element);
			if (k >= 3) {
				k = 0;
