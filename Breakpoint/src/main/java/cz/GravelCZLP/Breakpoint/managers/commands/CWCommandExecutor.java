...
public class CWCommandExecutor implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		sb.append(ChatColor.YELLOW.toString()).append(ChatColor.BOLD.toString())
				...
				.append(today == null ? MessageType.COMMAND_CW_DAYNOTTAKEN.getTranslation().getValue()
						: MessageType.COMMAND_CW_FORMAT_DAY.getTranslation().getValue(
		...
		for (int i = 1; i <= 7; i++) {
			...
			sb.append('\n').append(ChatColor.YELLOW.toString()).append(CWScheduler.getDayName(dayOfWeek)).append(": ")
					.append(day == null ? MessageType.COMMAND_CW_DAYNOTTAKEN.getTranslation().getValue()
							: MessageType.COMMAND_CW_FORMAT_DAY.getTranslation().getValue(
