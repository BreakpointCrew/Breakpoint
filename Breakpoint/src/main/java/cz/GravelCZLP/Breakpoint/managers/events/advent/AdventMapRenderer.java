...
public class AdventMapRenderer extends BPMapRenderer {
	public static final byte[] COLOR_BACKGROUND = new byte[] { BPMapPalette.getColor(BPMapPalette.DARK_GRAY, 0),
			BPMapPalette.getColor(BPMapPalette.DARK_GRAY, 1), BPMapPalette.getColor(BPMapPalette.DARK_GRAY, 2),
			BPMapPalette.getColor(BPMapPalette.DARK_GRAY, 1), BPMapPalette.getColor(BPMapPalette.DARK_GRAY, 0) };
	...
	public AdventMapRenderer(AdventManager advm, int dayOfMonth) {
		this.advm = advm;
		this.iconHeight = (int) Math.ceil((double) AdventManager.LAST_DAY / (double) this.iconWidth);
		this.iconStartY = MAP_SIZE - (this.iconHeight * 18 - 1) - 10;
		ArrayList<AdventGift> gifts = advm.getGifts();
		this.icons = loadIcons(gifts);
		this.numbers = loadNumbers(gifts.size());
		this.header = MessageType.EVENT_ADVENT_MAP_HEADER.getTranslation().getValue();
		this.description = MessageType.EVENT_ADVENT_MAP_DESCRIPTION.getTranslation().getValue();
	...
	public void drawGlow(MapCanvas canvas, boolean hasEarned) {
		...
		int iconX = this.iconStartX + i % 6 * 18;
	...
	public void drawIcons(MapCanvas canvas) {
		for (int i = 0; i < this.icons.length; i++) {
			...
			if (this.icons[i] != null && i < this.dayOfMonth) {
				for (int x = 0; x < this.icons[i].length; x++) {
					for (int y = 0; y < this.icons[i][x].length; y++) {
						int xx = iconX + x;
						int yy = iconY + y;
	...
	public void drawNumbers(MapCanvas canvas) {
		for (int i = 0; i < this.icons.length && i < this.dayOfMonth; i++) {
			if (this.icons[i] != null) {
				int numberX = this.iconStartX + i % 6 * 18 + (16 - this.numbers[i].length) / 2;
	...
	public void drawHeader(MapCanvas canvas) {
		int width = getWidth(this.header) * 2;
		int height = MinecraftFont.Font.getHeight() * 2;
		int startX = (MAP_SIZE - width) / 2;
		int startY = (this.iconStartY - 1 - height) / 3;

		drawText(canvas, startX, startY + 1, 2, 2, MinecraftFont.Font, this.header, COLOR_HEADER_BACKGROUND);
		drawText(canvas, startX, startY, 2, 2, MinecraftFont.Font, this.header, COLOR_HEADER);
	}

	public void drawDescription(MapCanvas canvas) {
		int width = getWidth(this.description);
		int height = MinecraftFont.Font.getHeight();
		int startX = (MAP_SIZE - width) / 2;
		int startY = (this.iconStartY - 1 - height) / 4 * 3;

		drawText(canvas, startX, startY + 1, 1, 1, MinecraftFont.Font, this.description, COLOR_DESCRIPTION_BACKGROUND);
		drawText(canvas, startX, startY, 1, 1, MinecraftFont.Font, this.description, COLOR_DESCRIPTION);
	}
	...
	private static void loadIcon(Byte[][][] icons, BPBlock block, int index) {
		...
		File file = new File("plugins/Breakpoint/images/textures/" + Material.getMaterial(id).name().toLowerCase()
				+ (data != 0 ? "_" + data : "") + ".png");
