...
public class RandomShop {
	...
	public RandomShop(Location location, int direction) {
		...
		this.costPerPiece = getRandomCostPerPiece(random);
		this.color = getRandomColor(random);
	}

	public void build() {
		int[] cost = { this.costPerPiece, this.costPerPiece * 2, this.costPerPiece * 4 };
		int[] time = { 60, 120, 240 };
		Calendar cal = Calendar.getInstance(Locale.GERMANY);
		String name = ChatColor.BOLD + "" + ChatColor.DARK_BLUE + cal.get(Calendar.WEEK_OF_YEAR) + "# "
				+ cal.get(Calendar.YEAR);

		ShopManager.buildArmorShop(this.location, this.direction, this.color, cost, time, name);
	}
