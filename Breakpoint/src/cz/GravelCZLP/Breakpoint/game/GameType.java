...
import cz.GravelCZLP.Breakpoint.game.cw.CWListener;
import cz.GravelCZLP.Breakpoint.game.dm.DMGame;
import cz.GravelCZLP.Breakpoint.game.dm.DMListener;
import cz.GravelCZLP.Breakpoint.game.dm.DMMap;
...
public enum GameType {
	CTF(MessageType.GAMETYPE_CTF, CTFListener.class) {
		...
		public CTFGame loadGame(YamlConfiguration yml, String name, Location signLoc) {
			...
			Location teamSelLoc = new Location(Bukkit.getWorld(rawTeamSelLoc[0]), Double.parseDouble(rawTeamSelLoc[1]),
					Double.parseDouble(rawTeamSelLoc[2]), Double.parseDouble(rawTeamSelLoc[3]),
					Float.parseFloat(rawTeamSelLoc[4]), Float.parseFloat(rawTeamSelLoc[5]));

			String[] rawCharSelLoc = yml.getString(name + ".charSelLoc", "world,0,0,0,0,0").split(",");
			Location charSelLoc = new Location(Bukkit.getWorld(rawCharSelLoc[0]), Double.parseDouble(rawCharSelLoc[1]),
					Double.parseDouble(rawCharSelLoc[2]), Double.parseDouble(rawCharSelLoc[3]),
					Float.parseFloat(rawCharSelLoc[4]), Float.parseFloat(rawCharSelLoc[5]));

			String mapsPath = name + ".maps";
			ConfigurationSection mapsSection = yml.getConfigurationSection(mapsPath);
			Set<String> mapNames = mapsSection != null ? mapsSection.getKeys(false) : new HashSet<>();
			LinkedList<CTFMap> maps = new LinkedList<>();
	...
	DM(MessageType.GAMETYPE_DM, DMListener.class) {
		...
		public DMGame loadGame(YamlConfiguration yml, String name, Location signLoc) {
			...
			ConfigurationSection mapsSection = yml.getConfigurationSection(mapsPath);
			Set<String> mapNames = mapsSection != null ? mapsSection.getKeys(false) : new HashSet<>();
			LinkedList<DMMap> maps = new LinkedList<>();
	...
	CW(MessageType.GAMETYPE_CW, CWListener.class) {
		...
		public CWGame loadGame(YamlConfiguration yml, String name, Location signLoc) {
			...
			LinkedList<ClanChallenge> days = new LinkedList<>();
