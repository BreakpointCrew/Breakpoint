...
public class ClanCommandExecutor implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (args.length <= 0) {
			...
			if (args.length <= 1) {
				...
				if (bpClan == null) {
					sender.sendMessage(getCommandInfo(MessageType.COMMAND_CLAN_INFO_PATH.getTranslation().getValue(),
							MessageType.COMMAND_CLAN_INFO_DESC.getTranslation().getValue(), ChatColor.AQUA));
					return true;
				}
			} else {
			...
			StringBuilder moderatorString = new StringBuilder(), memberString = new StringBuilder(),
					invitedString = new StringBuilder(), pendingChallengesString = new StringBuilder(),
			...
			sender.sendMessage(MessageType.COMMAND_CLAN_INFO_EXE_SUCCESS.getTranslation().getValue(clanNameColored,
					getOnlinePlayerName(leader), moderatorString, memberString, invitedString, rank,
					pendingChallengesString, matchResultsString, bpClan.getWins(), bpClan.getDraws(), bpClan.getLoses(),
					bpClan.getPoints()));
			...
			if (args.length <= 1) {
				sender.sendMessage(getCommandInfo(MessageType.COMMAND_CLAN_JOIN_PATH.getTranslation().getValue(),
						MessageType.COMMAND_CLAN_JOIN_DESC.getTranslation().getValue(), ChatColor.AQUA));
			} else {
				String clanName = args[1];
				...
				if (bpClan != null) {
					...
					if (bpClan.isInvited(playerName)) {
						...
						player.sendMessage(MessageType.COMMAND_CLAN_JOIN_EXE_NOTINVITED.getTranslation()
								.getValue(clanNameColored));
					}
				} else {
			...
			if (curClan != null) {
				String clanName = curClan.getColoredName();
				player.sendMessage(
						MessageType.COMMAND_CLAN_CREATE_EXE_ALREADYJOINED.getTranslation().getValue(clanName));
				return true;
			}
			if (args.length <= 1) {
				player.sendMessage(getCommandInfo(MessageType.COMMAND_CLAN_CREATE_PATH.getTranslation().getValue(),
						MessageType.COMMAND_CLAN_CREATE_DESC.getTranslation().getValue(), ChatColor.AQUA));
			...
			if (!curClan.isLeader(playerName)) {
				player.sendMessage(
						MessageType.COMMAND_CLAN_RENAME_EXE_NOTLEADER.getTranslation().getValue(clanNameColored));
				return true;
			}
			if (args.length <= 1) {
				player.sendMessage(getCommandInfo(MessageType.COMMAND_CLAN_RENAME_PATH.getTranslation().getValue(),
						MessageType.COMMAND_CLAN_RENAME_DESC.getTranslation().getValue(), ChatColor.AQUA));
				player.sendMessage(MessageType.COMMAND_CLAN_RENAME_COLORHINT.getTranslation().getValue());
			} else {
				try {
				...
				player.sendMessage(MessageType.COMMAND_CLAN_RENAME_EXE_SUCCESS.getTranslation()
						.getValue(curClan.getColoredName()));
			}
			...
			if (curClan == null) {
				...
				return true;
			} else if (!b && !curClan.isModerator(playerName)) {
			...
			if (!curClan.isAtLeastModerator(playerName)) {
				player.sendMessage(
						MessageType.COMMAND_CLAN_INVITE_EXE_NOTMODERATOR.getTranslation().getValue(clanNameColored));
				return true;
			}
			if (args.length <= 1) {
				player.sendMessage(getCommandInfo(MessageType.COMMAND_CLAN_INVITE_PATH.getTranslation().getValue(),
						MessageType.COMMAND_CLAN_INVITE_DESC.getTranslation().getValue(), ChatColor.AQUA));
			} else {
				...
				if (curClan.isAtLeastMember(targetName)) {
					player.sendMessage(
							MessageType.COMMAND_CLAN_INVITE_EXE_ALREADYMEMBER.getTranslation().getValue(targetName));
					return true;
				}
				Player target = Bukkit.getPlayerExact(targetName);
				if (curClan.isInvited(targetName)) {
					curClan.removeInvited(targetName);
					player.sendMessage(MessageType.COMMAND_CLAN_INVITE_EXE_SUCCESS_TAKENDOWN_SENDER.getTranslation()
							.getValue(targetName));

					if (target != null) {
						target.sendMessage(MessageType.COMMAND_CLAN_INVITE_EXE_SUCCESS_TAKENDOWN_TARGET.getTranslation()
								.getValue(clanNameColored));
					}
				} else {
					String clanName = ChatColor.stripColor(clanNameColored);
					curClan.addInvited(targetName);
					player.sendMessage(MessageType.COMMAND_CLAN_INVITE_EXE_SUCCESS_CREATE_SENDER.getTranslation()
							.getValue(targetName));

					if (target != null) {
						target.sendMessage(MessageType.COMMAND_CLAN_INVITE_EXE_SUCCESS_CREATE_TARGET.getTranslation()
								.getValue(clanNameColored, clanName));
					}
				}
			}
			...
			if (args.length <= 1) {
				player.sendMessage(getCommandInfo(MessageType.COMMAND_CLAN_KICK_PATH.getTranslation().getValue(),
						MessageType.COMMAND_CLAN_KICK_DESC.getTranslation().getValue(), ChatColor.AQUA));
			} else {
				String targetName = args[1];
				if (curClan.isAtLeastMember(targetName) && !curClan.isLeader(targetName)) {
					...
					player.sendMessage(
							MessageType.COMMAND_CLAN_KICK_EXE_TARGETNOTMEMBER.getTranslation().getValue(targetName));
				}
			}
			...
			if (args.length <= 1) {
				player.sendMessage(getCommandInfo(MessageType.COMMAND_CLAN_MODERATOR_PATH.getTranslation().getValue(),
						MessageType.COMMAND_CLAN_MODERATOR_DESC.getTranslation().getValue(), ChatColor.AQUA));
			} else {
				String targetName = args[1];
				...
				if (curClan.isAtLeastMember(targetName) && !curClan.isLeader(targetName)) {
					if (curClan.isMember(targetName)) {
						...
						if (target != null) {
							...
							target.sendMessage(MessageType.COMMAND_CLAN_MODERATOR_EXE_UPGRADED_TARGET.getTranslation()
									.getValue(clanNameColored));
						}

						sender.sendMessage(MessageType.COMMAND_CLAN_MODERATOR_EXE_UPGRADED_SENDER.getTranslation()
								.getValue(targetName));
					} else {
						...
						if (target != null) {
							...
							target.sendMessage(MessageType.COMMAND_CLAN_MODERATOR_EXE_DOWNGRADED_TARGET.getTranslation()
									.getValue(clanNameColored));
						}

						sender.sendMessage(MessageType.COMMAND_CLAN_MODERATOR_EXE_DOWNGRADED_SENDER.getTranslation()
								.getValue(targetName));
					}
				} else {
					player.sendMessage(MessageType.COMMAND_CLAN_MODERATOR_EXE_TARGETNOTMEMBER.getTranslation()
							.getValue(targetName));
				}
			}
			...
			if (args.length <= 3) {
				player.sendMessage(getCommandInfo(MessageType.COMMAND_CLAN_CHALLENGE_PATH.getTranslation().getValue(),
						MessageType.COMMAND_CLAN_CHALLENGE_DESC.getTranslation().getValue(), ChatColor.AQUA));
			} else {
				Clan bpClan = Clan.get(args[1]);

				if (bpClan == null) {
					sender.sendMessage(
							MessageType.COMMAND_CLAN_CHALLENGE_EXE_NOTFOUND.getTranslation().getValue(args[1]));
					return true;
				} else if (bpClan.equals(curClan)) {
					sender.sendMessage(
							MessageType.COMMAND_CLAN_CHALLENGE_EXE_CHALLENGEDITSELF.getTranslation().getValue());
					return false;
				}
				...
				if (dayOfWeek == null) {
					sender.sendMessage(
							MessageType.COMMAND_CLAN_CHALLENGE_EXE_UNKNOWNDAY.getTranslation().getValue(args[2]));
					return true;
				}
				...
				if (day != null) {
					String challengingName = day.getChallengingClan() != null
							? day.getChallengingClan().getColoredName() : "?";
					String challengedName = day.getChallengedClan() != null ? day.getChallengedClan().getColoredName()
							: "?";
					sender.sendMessage(MessageType.COMMAND_CLAN_CHALLENGE_EXE_DAYTAKEN.getTranslation()
							.getValue(challengingName, challengedName));
					return true;
				}
				...
				try {
					...
					sender.sendMessage(
							MessageType.COMMAND_CLAN_CHALLENGE_EXE_INCORRECTMAXPLAYERS.getTranslation().getValue());
					return false;
				}

				if (maxPlayers < 1) {
					sender.sendMessage(
							MessageType.COMMAND_CLAN_CHALLENGE_EXE_TOOFEWMAXPLAYERS.getTranslation().getValue());
					return false;
				}

				if (curClan.getPlayerNames().size() < maxPlayers) {
					sender.sendMessage(MessageType.COMMAND_CLAN_CHALLENGE_EXE_MAXPLAYERSFEWPLAYERS_OWN.getTranslation()
							.getValue());
					return false;
				}

				if (bpClan.getPlayerNames().size() < maxPlayers) {
					sender.sendMessage(MessageType.COMMAND_CLAN_CHALLENGE_EXE_MAXPLAYERSFEWPLAYERS_THEIR
							.getTranslation().getValue());
					return false;
				}
				...
				if (theirChallenge != null) {
					...
					sender.sendMessage(
							MessageType.COMMAND_CLAN_CHALLENGE_EXE_ALREADYCHALLENGED.getTranslation().getValue(curDay));
					return true;
				}
				...
				if (ourChallenge != null) {
					...
					sender.sendMessage(
							MessageType.COMMAND_CLAN_CHALLENGE_EXE_CHALLENGEPENDING.getTranslation().getValue(curDay));
					return true;
				}
				...
				sender.sendMessage(MessageType.COMMAND_CLAN_CHALLENGE_EXE_SUCCESS.getTranslation()
						.getValue(bpClan.getColoredName(), dayName, maxPlayers));

				if (leader != null) {
					leader.sendMessage(MessageType.COMMAND_CLAN_CHALLENGE_EXE_SUCCESSOTHER.getTranslation()
							.getValue(curClan.getColoredName(), dayName, maxPlayers));
				}
			}
			...
			if (args.length <= 1) {
				player.sendMessage(getCommandInfo(MessageType.COMMAND_CLAN_ACCEPT_PATH.getTranslation().getValue(),
						MessageType.COMMAND_CLAN_ACCEPT_DESC.getTranslation().getValue(), ChatColor.AQUA));
			} else {
				Clan bpClan = Clan.get(args[1]);
				...
				if (ourChallenge == null) {
					sender.sendMessage(MessageType.COMMAND_CLAN_ACCEPT_EXE_NOCHALLENGE.getTranslation()
							.getValue(bpClan.getColoredName()));
					return true;
				}
				...
				sender.sendMessage(MessageType.COMMAND_CLAN_ACCEPT_EXE_SUCCESS.getTranslation()
						.getValue(bpClan.getColoredName(), dayName));

				if (leader != null) {
					leader.sendMessage(MessageType.COMMAND_CLAN_ACCEPT_EXE_SUCCESSOTHER.getTranslation()
							.getValue(curClan.getColoredName(), dayName));
				}
			}
			...
			if (args.length <= 1) {
				player.sendMessage(getCommandInfo(MessageType.COMMAND_CLAN_REJECT_PATH.getTranslation().getValue(),
						MessageType.COMMAND_CLAN_REJECT_DESC.getTranslation().getValue(), ChatColor.AQUA));
			} else {
				Clan bpClan = Clan.get(args[1]);
				...
				if (ourChallenge == null) {
					sender.sendMessage(MessageType.COMMAND_CLAN_REJECT_EXE_NOCHALLENGE.getTranslation()
							.getValue(bpClan.getColoredName()));
					return true;
				}
				...
				curClan.getPendingChallenges().remove(ourChallenge);
				sender.sendMessage(
						MessageType.COMMAND_CLAN_REJECT_EXE_SUCCESS.getTranslation().getValue(bpClan.getColoredName()));

				if (leader != null) {
					leader.sendMessage(MessageType.COMMAND_CLAN_REJECT_EXE_SUCCESSOTHER.getTranslation()
							.getValue(curClan.getColoredName()));
				}
			}
		} else {
	...
	private static void listClanCommands(CommandSender sender, ChatColor color) {
		sender.sendMessage(getCommandInfo(MessageType.COMMAND_CLAN_INFO_PATH.getTranslation().getValue(),
				MessageType.COMMAND_CLAN_INFO_DESC.getTranslation().getValue(), color));
		sender.sendMessage(getCommandInfo(MessageType.COMMAND_CLAN_JOIN_PATH.getTranslation().getValue(),
				MessageType.COMMAND_CLAN_JOIN_DESC.getTranslation().getValue(), color));
		sender.sendMessage(getCommandInfo(MessageType.COMMAND_CLAN_LEAVE_PATH.getTranslation().getValue(),
				MessageType.COMMAND_CLAN_LEAVE_DESC.getTranslation().getValue(), color));
		sender.sendMessage(getCommandInfo(MessageType.COMMAND_CLAN_CREATE_PATH.getTranslation().getValue(),
				MessageType.COMMAND_CLAN_CREATE_DESC.getTranslation().getValue(), color));
		sender.sendMessage(getCommandInfo(MessageType.COMMAND_CLAN_RENAME_PATH.getTranslation().getValue(),
				MessageType.COMMAND_CLAN_RENAME_DESC.getTranslation().getValue(), color));
		sender.sendMessage(getCommandInfo(MessageType.COMMAND_CLAN_MODERATOR_PATH.getTranslation().getValue(),
				MessageType.COMMAND_CLAN_MODERATOR_DESC.getTranslation().getValue(), color));
		sender.sendMessage(getCommandInfo(MessageType.COMMAND_CLAN_INVITE_PATH.getTranslation().getValue(),
				MessageType.COMMAND_CLAN_INVITE_DESC.getTranslation().getValue(), color));
		sender.sendMessage(getCommandInfo(MessageType.COMMAND_CLAN_KICK_PATH.getTranslation().getValue(),
				MessageType.COMMAND_CLAN_KICK_DESC.getTranslation().getValue(), color));
		sender.sendMessage(getCommandInfo(MessageType.COMMAND_CLAN_CHALLENGE_PATH.getTranslation().getValue(),
				MessageType.COMMAND_CLAN_CHALLENGE_DESC.getTranslation().getValue(), color));
		sender.sendMessage(getCommandInfo(MessageType.COMMAND_CLAN_ACCEPT_PATH.getTranslation().getValue(),
				MessageType.COMMAND_CLAN_ACCEPT_DESC.getTranslation().getValue(), color));
		sender.sendMessage(getCommandInfo(MessageType.COMMAND_CLAN_REJECT_PATH.getTranslation().getValue(),
				MessageType.COMMAND_CLAN_REJECT_DESC.getTranslation().getValue(), color));
	}
