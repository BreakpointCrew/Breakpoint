...
public class AchievementsCommandExecutor implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (args.length <= 0) {
			sender.sendMessage(getCommandInfo(MessageType.COMMAND_ACHIEVEMENTS_PATH.getTranslation().getValue(),
					MessageType.COMMAND_ACHIEVEMENTS_DESC.getTranslation().getValue(), ChatColor.AQUA));
		} else {
